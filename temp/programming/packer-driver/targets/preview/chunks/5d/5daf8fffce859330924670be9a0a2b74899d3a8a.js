  ...
  return {
    ...
    execute: function () {
      ...
      _export("PyramidDrawLayer", PyramidDrawLayer = (_dec = ccclass('PyramidDrawLayer'), _dec2 = property({
        ...
        initItemState() {
          for (var index = 0; index < 10 && index < this.items.length; index++) {
            var _drawItem = this.items[index];
            _drawItem.getChildByName("grayMask").active = (_crd && UserData === void 0 ? (_reportPossibleCrUseOfUserData({
              error: Error()
            }), UserData) : UserData).getInstance().$pyramidState[index] == 1;
            _drawItem.getChildByName("xuanzhong").active = false;
          }

          var drawItem = (_crd && designManager === void 0 ? (_reportPossibleCrUseOfdesignManager({
            error: Error()
        ...
        update(dt) {
          ...
          if (this._dtTime > 0.1) {
            ...
            var itemsIndex = this.getDrawIndex(this._index, this._index >= this._endIndex);
            this.items[itemsIndex].getChildByName("xuanzhong").active = true;

            if (this._index >= this._endIndex) {
              this._isStartRun = false;
              var curBet = this._drawItemArr[itemsIndex][0];
              var propItem = {
                Id: curBet[0],
        ...
        getDrawIndex(index, isHero) {
          ...
          for (var i = isHero ? 1 : 0; i < (_crd && UserData === void 0 ? (_reportPossibleCrUseOfUserData({
            error: Error()
...
//# sourceMappingURL=5daf8fffce859330924670be9a0a2b74899d3a8a.js.map