    ...
    execute: function () {
      ...
      _export("Monster", Monster = (_dec = ccclass('Monster'), _dec(_class = class Monster extends Component {
        ...
        onLoad() {
          ...
          var hpProgressNode = find("hpProgress", this.node);
        ...
        setBodySkin() {
          var sk = this.bodyNode.getComponent(sp.Skeleton);
        ...
        init(id) {
          ...
          if ((_crd && mapModel === void 0 ? (_reportPossibleCrUseOfmapModel({
            ...
            var hp = this.hp * (1 + floorRow.hp / 100);
          ...
          if (row.type != (_crd && constants === void 0 ? (_reportPossibleCrUseOfconstants({
            ...
            for (var i = 0; i < arr.length; i++) {
              ...
              if ((_crd && mapModel === void 0 ? (_reportPossibleCrUseOfmapModel({
                error: Error()
              }), mapModel) : mapModel).time <= _row.time) {
                addRow = _row;
                break;
              }
            }
          ...
          try {
            ...
            var monsterUtil = (_crd && cocosUtil === void 0 ? (_reportPossibleCrUseOfcocosUtil({
              error: Error()
        ...
        playerForceMove(movePos) {
          ...
          var radian = Vec2.angle(playerPos, movePos);
        ...
        monsterForceMove(monster, movePos) {
          ...
          var monsterPos = monster.node.getPosition();
          var pos = this.node.getPosition();
          monsterPos.subtract(pos).negative(); // 只有挡在怪物前进方向的怪物才需要推开

          var radian = Vec2.angle(monsterPos, movePos);
        ...
        hitWithDam(dam, shineColor) {
          ...
          if (this.shineColor && this.row.type != (_crd && constants === void 0 ? (_reportPossibleCrUseOfconstants({
            ...
            // 闪白
            var materialPath = (_crd && constants === void 0 ? (_reportPossibleCrUseOfconstants({
              error: Error()
        ...
        updateLogic(dt) {
          ...
          for (var i = 0; i < 2; i++) {
            ...
            var mp = dir.multiplyScalar(this.raycastDis);
            (_crd && cocosUtil === void 0 ? (_reportPossibleCrUseOfcocosUtil({
              error: Error()
            }), cocosUtil) : cocosUtil).vec3CopyVal(this.vec3Obj.vec3Temp, this.vec3Obj.nodePos);
            var endPos = this.vec3Obj.vec3Temp.add(mp);
            ...
            var colliderArr = PhysicsSystem2D.instance.testPoint(v2(endPos.x, endPos.y));
            var has = false;

            for (var k = 0; k < colliderArr.length; k++) {
              var collider = colliderArr[k];
        ...
        move(dt) {
          ...
          var dis = speed * dt;
          var movePos = this.targetDirection.normalize().multiplyScalar(dis);
...
//# sourceMappingURL=f44d12c88dbf258d295d3be17ac6cc314bb78f20.js.map