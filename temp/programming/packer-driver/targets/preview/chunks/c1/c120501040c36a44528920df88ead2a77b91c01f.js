    ...
    execute: function () {
      ...
      _export("RedPointLogicMgr", RedPointLogicMgr = class RedPointLogicMgr {
        ...
        initGame() {
          ...
          var keys = Object.keys(RPointMask);
        ...
        onCMSGRPValueSetting(_, param) {
          var rp = param.get(); // 如果传过来的红点为True，则表示必定红点；如果为false,则需要重新计算
          ...
          if (rp.value == true) {
            ...
            if (rp.forceStop == true) {
              var _checkIndex = this._waitChecks.indexOf(rp.mask);

              if (_checkIndex != INVALID_VALUE) {
                this._waitChecks.splice(_checkIndex, 1);
              }
        ...
        updateVisit() {
          ...
          var mask_value = this._redPointMask.get(mask); // 取出所有需要变更的节点
        ...
        setRPointView(parent, subPath, value, effectType, posType, cb) {
          ...
          var aniNode = node.getChildByName(RED_POINT_NAME);
...
//# sourceMappingURL=c120501040c36a44528920df88ead2a77b91c01f.js.map