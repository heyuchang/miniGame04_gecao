  ...
  return {
    ...
    execute: function () {
      ...
      _export("Player", Player = (_dec = ccclass('Player'), _dec(_class = class Player extends (_crd && BaseLayer === void 0 ? (_reportPossibleCrUseOfBaseLayer({
        ...
        addHpMaxPercent(p) {
          var lastHpMax = this.hpMax;
          var nowHpMax = this.hpMaxOrign * (1 + p);
          nowHpMax = Math.round(nowHpMax);
          var addHp = nowHpMax - lastHpMax;
          this.hpMax = nowHpMax;
          this.addHp(addHp);
        } // 增加血量百分比
        ...
        addMonster(collider) {
          ...
          var monster = collider.getComponent(_crd && Monster === void 0 ? (_reportPossibleCrUseOfMonster({
            error: Error()
          }), Monster) : Monster);
          var index = this.monsterArr.indexOf(monster);
        ...
        move(dt) {
          ...
          var nowPos = this.node.getPosition(this.outVec3);
          var dis = this.getMoveSpeed() * dt;
          var movePos = this.movePos.clone().multiplyScalar(dis);
          nowPos.add(movePos);
          this.node.setPosition(nowPos);
          (_crd && mapModel === void 0 ? (_reportPossibleCrUseOfmapModel({
            error: Error()
          }), mapModel) : mapModel).mapSystem.adjustInWall(this.node);
          var scale = this.bodyNode.getScale();
...
//# sourceMappingURL=26981a71e68d4f567d21f697ce11cdb431f9dcd8.js.map