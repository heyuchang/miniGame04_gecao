    ...
    execute: function () {
      ...
      _export("NoticeLayer", NoticeLayer = (_dec = ccclass('NoticeLayer'), _dec(_class = class NoticeLayer extends Component {
        ...
        noticeShow(content) {
          ...
          node.active = true;
          var contentNode = find("content", node);
          ...
          var width = contentNode.getComponent(UITransform).width;
        ...
        update(dt) {
          var chs = this.contentLayer.children;
          var backArr = [];
          ...
          for (var _i in backArr) {
            var _node = backArr[_i];
            _node.parent = null;
            this.textPool.put(_node);
          } // 修正位置，保证不重叠
          ...
          if (chs.length >= 2) {
            var lastNode = chs[chs.length - 1];

            for (var _i2 = chs.length - 2; _i2 >= 0; _i2--) {
              var _node2 = chs[_i2];

              var _dy = (_node2.getComponent(UITransform).height + lastNode.getComponent(UITransform).height) * 0.5 + this.spaceY;

              if (_node2.position.y - lastNode.position.y < _dy) {
                var _pos = _node2.getPosition();

                _pos.y = lastNode.position.y + _dy;

                _node2.setPosition(_pos);
              }
...
//# sourceMappingURL=64ef452e31d74908de983365e189150cbaef5230.js.map