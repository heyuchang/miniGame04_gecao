    ...
    execute: function () {
      ...
      PhysicsWorld = class PhysicsWorld {
        ...
        forColliderRectKeyArr(collider, cb) {
          ...
          var minX = Math.floor(rect.x / this.cellSize);
          var maxX = Math.ceil((rect.x + rect.width) / this.cellSize);
          var minY = Math.floor(rect.y / this.cellSize);
          var maxY = Math.ceil((rect.y + rect.height) / this.cellSize);

          for (var i = minX; i <= maxX; i++) {
            for (var j = minY; j <= maxY; j++) {
              var key = this.getPosKey(i, j);
              cb(key);
            }
          }
        }
        ...
        addIntoPosToColliderArr(collider) {
          this.forColliderRectKeyArr(collider, key => {
            var arr = this.posToColliderArr[key];
        ...
        collectCollider(node) {
          ...
          var arr = node.getComponents(_crd && BoxColliderUtil === void 0 ? (_reportPossibleCrUseOfBoxColliderUtil({
            error: Error()
        ...
        doIntersection(collider1, collider2) {
          ...
          if (collisionMatrix[collider1.group] & collider2.group && collisionMatrix[collider2.group] & collider1.group) {
            var ret = Intersection2D.polygonPolygon(collider1.worldPoints, collider2.worldPoints);
...
//# sourceMappingURL=a2bbba34157578c073d563cb953ae8cec0de77ce.js.map