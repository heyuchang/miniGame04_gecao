System.register(["__unresolved_0", "cc", "__unresolved_1"], function (_export, _context) {
  ...
  var _reporterNs, _cclegacy, __checkObsolete__, __checkObsoleteInNamespace__, Button, Color, Input, input, instantiate, isValid, Node, Quat, sys, tween, Tween, UIOpacity, UIRenderer, UITransform, v3, Vec3, view, utilTools, _crd, cocosUtil;
  ...
  return {
    ...
    execute: function () {
      ...
      _cclegacy._RF.push({}, "c6b74pmm/VEqaWZqrRtczgX", "cocosUtil", undefined);
      ...
      Object.defineProperty(Node.prototype, "opacity", {
        ...
        get() {
          var op = cocosUtil.addComponentOnce(this, UIOpacity);
          return op.opacity;
        },
      ...
      _export("cocosUtil", cocosUtil = {
         ...
         */
        getAroundPos(node, minRadius, maxRadius, isWorld) {
          if (isWorld === void 0) {
            isWorld = false;
          }

          var radius = (_crd && utilTools === void 0 ? (_reportPossibleCrUseOfutilTools({
            error: Error()
          }), utilTools) : utilTools).getFloatValue(minRadius, maxRadius, true);
          var radian = Math.random() * Math.PI * 2;
          var pos = v3(radius, 0, 0);
         ...
         * @param isWorld 是否转为世界坐标，默认：是node坐标系下
         */
        getRectPos(node, minWidth, minHeight, maxWidth, maxHeight, isWorld) {
          if (isWorld === void 0) {
            isWorld = false;
          }
          ...
          r4.weight = r3.weight;
          arr.push(r4);
          var r = (_crd && utilTools === void 0 ? (_reportPossibleCrUseOfutilTools({
            error: Error()
          }), utilTools) : utilTools).getRowByWeight(arr);
          var x = r.x + Math.random() * r.width;
          var y = r.y + Math.random() * r.height;
          var pos = v3(x, y);
          pos = node.getPosition().add(pos);
        ...
        schedule(com, cb, interval, repeatNum, delayTime, isAtOnce) {
          if (delayTime === void 0) {
            delayTime = 0;
          }
        ...
        addComponentOnce(item, name) {
          var com = item.getComponent(name);
         ...
         * @param radius 向量长度
         */
        getSectorDirectionArr(direction, angle, num, radius) {
          if (radius === void 0) {
            radius = 1;
          }
          ...
          for (var i = da; i >= 1; i--) {
            var dir1 = new Vec3(0, 0, 0);
            var dr = radian * i;
            Vec3.rotateZ(dir1, direction, Vec3.ZERO, dr);
            topArr.push(dir1);
            var dir2 = new Vec3(0, 0, 0);
            dr = -radian * (da + 1 - i);
         ...
         * @param radius 向量长度
         */
        getCircleDirectionArr(num, radius) {
          if (radius === void 0) {
            radius = 1;
          }

          var r = Math.PI * 2 / num;
          var p2 = v3(radius, 0, 0);
          var arr = []; // 随机起始角度

          var startRadian = Math.random() * Math.PI * 2;

          for (var i = 0; i < num; i++) {
            var radian = startRadian + r * i;
            var direction = v3(0, 0, 0);
        ...
        isInScreenVisible(node) {
          ...
          var orignPos = view.getVisibleOrigin();
          var pos = cocosUtil.convertToWorldSpace(node);
          var height = view.getVisibleSize().height;
          var width = view.getVisibleSize().width;
          var tf = node.getComponent(UITransform); // 考虑锚点
        ...
        tweenFlyToPos(node, pos, endCb) {
          return _asyncToGenerator(function* () {
            ...
            return new Promise((resolve, reject) => {
              var time = 0.5;
              var angle = 360;
              var quat = cocosUtil.getQuatByAngleZ(angle);
        ...
        tweenBlink(node, time, cb) {
          ...
          var uiOpacity = this.addComponentOnce(node, UIOpacity);
          uiOpacity.opacity = 255;
        ...
        tweenFadeIn(node, time, cb) {
          ...
          return _asyncToGenerator(function* () {
            return new Promise((resolve, reject) => {
              ...
              var uiOpacity = _this.addComponentOnce(node, UIOpacity);
        ...
        tweenFadeOut(node, time, cb) {
          ...
          return _asyncToGenerator(function* () {
            return new Promise((resolve, reject) => {
              ...
              var uiOpacity = _this2.addComponentOnce(node, UIOpacity);
        ...
        intiScreenAdapterWidthScale() {
          var fh = this.frameSize.height;
          var fw = this.frameSize.width;
          var scale = fh / this.designSize.height;
          var adapterWidth = this.designSize.width * scale;
          var retScale = 1;
         ...
         */
        convertToWorldSpace(node, isCenter) {
          var transform = node.parent.getComponent(UITransform);
          var pos = node.getPosition();
         ...
         */
        getColorByHEX(hex) {
          var color = new Color(hex); // Color.fromHEX(color, hex);
...
//# sourceMappingURL=99e3dce5c205211055dbf990b529b3d5dbd99393.js.map