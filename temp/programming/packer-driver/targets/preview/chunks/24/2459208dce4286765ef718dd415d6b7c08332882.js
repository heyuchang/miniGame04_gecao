    ...
    execute: function () {
      ...
      _export("ShineColor", ShineColor = (_dec = ccclass('ShineColor'), _dec(_class = class ShineColor extends Component {
        ...
        setMaterialProperty(val) {
          if (this.isSpine) {
            // @ts-ignore 
            var cache = this.uiRenderer._materialCache;

            for (var i in cache) {
              var _material = cache[i];

              _material.setProperty("u_rate", val);

              if (this.vColor) {
                _material.setProperty("u_color", this.vColor);
              }
            }
          ...
          var material = this.uiRenderer.getMaterialInstance(0);
          var uRate = material.passes[0].getHandle("u_rate");
          material.passes[0].setUniform(uRate, val);

          if (this.vColor) {
            var vColor = v4(1, 0, 0, 1);
            var uColor = material.passes[0].getHandle("u_color");
        ...
        startShine(materialPath, color, duration) {
          if (duration === void 0) {
            duration = 0.3;
          }
...
//# sourceMappingURL=2459208dce4286765ef718dd415d6b7c08332882.js.map