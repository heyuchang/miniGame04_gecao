  ...
  return {
    ...
    execute: function () {
      ...
      _export("bullet1101", bullet1101 = (_dec = ccclass('bullet1101'), _dec(_class = class bullet1101 extends Component {
        ...
        shot() {
          var pos = v3(this.targetDis + (Math.random() - 0.5) * 50, 0, 0);
          ...
          var targetPos = (_crd && mapModel === void 0 ? (_reportPossibleCrUseOfmapModel({
            error: Error()
          }), mapModel) : mapModel).player.node.getPosition().add(pos); // 发射角度

          var direction = targetPos.clone().subtract(this.node.getPosition());
          var radian = (Math.random() - 0.5) * (_crd && utilTools === void 0 ? (_reportPossibleCrUseOfutilTools({
            error: Error()
        ...
        update(dt) {
          ...
          if (dis > 5) {
            speed = dis * 2;
            var maxSpeed = (_crd && mapModel === void 0 ? (_reportPossibleCrUseOfmapModel({
              error: Error()
          ...
          if (speed > 0) {
            ...
            var direction = playerPos.subtract(pos).normalize();
...
//# sourceMappingURL=07d4178e999c78b39c5876c645265994edfee75f.js.map