    ...
    execute: function () {
      ...
      _export("Bullet", Bullet = (_dec = ccclass('Bullet'), _dec(_class = class Bullet extends Component {
        ...
        init(id, group, startWorldPos, direction, extraObj) {
          ...
          try {
            var bulletUtilStr = this.node.name;
            ...
            var bulletUtil = (_crd && cocosUtil === void 0 ? (_reportPossibleCrUseOfcocosUtil({
              error: Error()
        ...
        contactEnd(selfCollider, otherCollider) {
          ...
          if (contactItem) {
            var index = this.contactItemArr.indexOf(contactItem);
        ...
        update(dt) {
          ...
          if (canMove) {
            var dis = this.speed * dt;
            var pos = this.node.position;

            if (this.isFollow) {
              // 根据转速，慢慢调整角度
              var targetPos = this.getFollowTargetPos();
              var direction = targetPos.subtract(pos).normalize();
              var radian1 = Math.atan2(direction.y, direction.x);

              if (radian1 < 0) {
                radian1 += Math.PI * 2;
              }

              var radian2 = Math.atan2(this.direction.y, this.direction.x);

              if (radian2 < 0) {
                radian2 += Math.PI * 2;
              }

              var dr = this.rotateSpeed * dt;
              var maxDr = Math.abs(radian1 - radian2);
              ...
              var _angle = (_crd && utilTools === void 0 ? (_reportPossibleCrUseOfutilTools({
                error: Error()
              }), utilTools) : utilTools).radianToAngle(Math.atan2(this.direction.y, this.direction.x));
            ...
            if (this.bulletUtil && this.bulletUtil.targetPos && this.bulletUtil.arriveTargetPos) {
              // 需要判断是否到达目的地
              var _targetPos = this.getFollowTargetPos();

              lastDirection = _targetPos.subtract(pos);
            }
            ...
            if (lastDirection) {
              var _targetPos2 = this.getFollowTargetPos();

              var afterDirection = _targetPos2.subtract(pos);
...
//# sourceMappingURL=94d90d1f80acb49b52f5f196d093dd42dc7369c1.js.map