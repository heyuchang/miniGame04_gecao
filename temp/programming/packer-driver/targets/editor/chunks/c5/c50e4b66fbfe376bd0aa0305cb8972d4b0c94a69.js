  ...
  return {
    ...
    execute: function () {
      ...
      networkMgr = class networkMgr {
        ...
        xhrPost(_route, data, cb, fail, isRepeat = false) {
          this.mid += 1; // 包
        ...
        checkRespone(route, _respone, mid) {
          ...
          let data = respone; // if (respone.result) {
          ...
          let message = this.messageList[mid]; // 头部回包处理
        ...
        sendMessage(mid, isCheck = true) {
          // 消息队列为空
          if (Object.keys(this.messageList).length === 0) {
            return;
          }

          let message = this.messageList[mid]; // 一条消息的发送次数大于限制次数，建议重启游戏
...
//# sourceMappingURL=c50e4b66fbfe376bd0aa0305cb8972d4b0c94a69.js.map