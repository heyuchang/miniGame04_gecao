  ...
  return {
    ...
    execute: function () {
      ...
      _export("cocosUtil", cocosUtil = {
        ...
        // 受击缩放特效
        tweenHitScale(node, time = 0.1, endCb) {
        ...
        // 边旋转，边飞向目标位置
        async tweenFlyToPos(node, pos, endCb) {
          Tween.stopAllByTarget(node);
        ...
        // 淡入动画
        async tweenFadeIn(node, time, cb) {
          return new Promise((resolve, reject) => {
            if (!time) {
              time = 1;
            }
        ...
        // 淡出动画
        async tweenFadeOut(node, time, cb) {
          return new Promise((resolve, reject) => {
            if (!time) {
              time = 1.5;
            }
...
//# sourceMappingURL=c2c977c72db76c574b230b76d73267724dc0719e.js.map